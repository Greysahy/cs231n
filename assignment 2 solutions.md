...
3. BN对batch_size的大小有一定要求。一般来讲，batch_size越大，使用BN的效果越好

**实现Dropout**

训练时，设某个神经元输出为x,则其输出的期望 `E(train) = p * x + (1 - p) * 0 = px`

为了使测试时每个神经元的输出期望与训练时一致，可以考虑两种方法：

1. 训练时每个神经元的输出除以p，`E(train / p) = px / p = x`
2. 测试时每个神经元的输出乘p, `E(test * p) = x * p = px `

**前向传播**

实现的关键点就是生成dropout掩码:`mask = np.random.rand(*x.shape) < p`

1. `np.random.rand(*x.shape)`：这是NumPy库的一个函数调用，它会生成一个与数组`x`形状相同的随机数数组。`np.random.rand`函数返回一个0到1之间的均匀分布的随机数数组。`np.random.rand`不接受元组类型的参数作为`size`.`*x.shape`是用于解包`x`数组的形状，使得`np.random.rand`函数生成与`x`具有相同形状的随机数数组。

2. `< p`：这是一个比较操作，其中`p`是一个介于0到1之间的概率值（例如0.25表示25%的概率）。`<`符号是小于的意思。对于每个在`np.random.rand`生成的随机数数组中的元素，都会执行该比较操作。如果随机数小于`p`，则该元素的值为True，否则为False。

3. `mask = ...`：这是将比较结果赋值给名为`mask`的新变量的操作。`mask`将成为一个与`x`数组形状相同的布尔（True/False）数组，其中元素为True表示对应位置的随机数小于`p`，否则为False。

因此，整个代码的目的是生成一个与`x`数组形状相同的布尔掩码数组`mask`，其中`mask`中的元素是根据概率`p`随机生成的，并且`mask`中为True的元素表示对应位置的随机数小于`p`，而False的元素表示对应位置的随机数大于或等于`p`。

**通过这种方法生成的掩码，以随机数生成时，0-1之间的每个数被取到地概率相等为前提，因此神经元实际存活的概率可能和p不严格相等**
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        mask = np.random.rand(size=x.shape) < p
        out = x * mask
