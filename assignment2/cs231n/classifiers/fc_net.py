...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        if self.num_layers > 2:
          for i in range(len(hidden_dims)):
            ...
            if self.normalization == "batchnorm":
              self.params[f'gamma{i + 2}'] = np.ones(hidden_dims[i + 1])
              self.params[f'beta{i + 2}'] = np.zeros(hidden_dims[i + 1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          ...
          if self.normalization == "batchnorm":
            X, cache = batchnorm_forward(X, self.params[f'gamma{i + 1}'], self.params[f'beta{i + 1}'], self.bn_params[i])
            forward_cache[f'bn{i + 1}'] = cache
          X, cache = relu_forward(X)
          forward_cache[f'relu{i + 1}'] = cache
          if self.use_dropout:
            X, cache = dropout_forward(X, self.dropout_param)
            forward_cache[f'dropout{i + 1}'] = cache
        ...
        for i in reversed(range(self.num_layers - 1)):
          if self.use_dropout:
            dx = dropout_backward(dx, forward_cache[f'dropout{i + 1}'])
          dx = relu_backward(dx, forward_cache[f'relu{i + 1}'])
          if self.normalization == "batchnorm": 
            dx, grads[f'gamma{i + 1}'], grads[f'beta{i + 1}'] = batchnorm_backward(dx, forward_cache[f'bn{i + 1}'])
