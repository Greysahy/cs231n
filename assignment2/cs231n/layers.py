...
def softmax_loss(x, y):
    ...
    N, C = x.shape
    x = np.exp(x)
    p = x / np.sum(x, axis=1).reshape(-1, 1)  # N, C
...
def layernorm_backward(dout, cache):
    ...
    dx_norm = gamma * dout
    dvar = np.sum(dx_norm * (x - batch_mean) / (-2 * np.sqrt(batch_var) * (batch_var)), axis=1, keepdims=True)
    dmean = np.sum(-dx_norm / np.sqrt(batch_var), axis=1, keepdims=True) + np.sum(-2 * dvar * (x - batch_mean), axis=1, keepdims=True) / N
...
def conv_backward_naive(dout, cache):
    ...
    # 计算dx N, C, H, W
    dx = np.zeros_like(x_pad)  # np.sum(所有和x做过乘法的w的F个通道的w * 其对应的dout)
    for out_channel in range(F):
        for i in range(H_):
          for j in range(W_):
            r = i * stride
